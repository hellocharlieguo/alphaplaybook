#!/usr/bin/env python3
"""
patch_voicecards_v341.py

Refreshes src/data/voiceCards.ts for the 2026-08-31 cycle.

Five edits:

  1. header comment  -- still claimed "Cycle 2026-08-17 -- NO FREEZE ... v3.3
     '2026-07-15-v3.3-coresat'" under an asOf of August 30. Two versions stale.
  2. interface comments -- tickers / bucket / wholeBucket / curated are NOT read
     by SignalRecap.tsx. Verified: renderVoiceCard renders theme.name and
     theme.editorial only. The comments claimed these drive chip rendering,
     which sent two separate reviews down the wrong path. Marked dead in place
     rather than deleted, because they are the spec for a feature not yet built.
  3. Visser  -- four themes renamed to v3.4 trend vocabulary, editorials
     rewritten from 8/29 Pomp + 8/30 solo.
  4. Camillo -- themes renamed, HOOD moved to Tokenized Rails, TTWO added,
     BE thesis carried forward. From 8/24-8/30 Grok output.
  5. ZaStocks -- refreshed from the 8/24-8/31 Grok output (a genuine new
     capture; the byte-identical repeat of the prior window did not recur).

Voice objects are located by brace matching on their `name:` line and replaced
whole. No re.DOTALL anywhere.
"""
...
SPAN_GUARD = {"Visser": (20, 70), "Camillo": (10, 60), "ZaStocks": (10, 60)}
...
def find_voice_span(src, voice):
    """Find `  {\\n    name: '<voice>',` and brace-match to its closing `},`."""
    marker = "  {\n    name: '%s'," % voice
    hits = src.count(marker)
    if hits != 1:
        fail("voice marker for '%s' found %d times, expected 1" % (voice, hits))
    ...
    while i < len(src):
        ...
        if in_str:
            ...
            if depth == 0:
                ...
                return start, end
        i += 1
    fail("unbalanced braces scanning voice '%s'" % voice)


def swap_voice(src, voice, new_body):
    start, end = find_voice_span(src, voice)
    n_old = src[start:end].count("\n") + 1
    lo, hi = SPAN_GUARD[voice]
    if not (lo <= n_old <= hi):
        fail("voice '%s' span is %d lines, outside expected %d-%d" % (voice, n_old, lo, hi))
    print("  %-9s %3d lines -> %3d lines" % (voice, n_old, new_body.count("\n") + 1))
...
# 1. header
# ---------------------------------------------------------------------------
OLD_HEADER = """// Cycle 2026-08-17 — NO FREEZE. Ticker set unchanged (v3.3 '2026-07-15-v3.3-coresat').
// No stage flip. CPI 3.4% → cap_multiple 1.0 dormant. CapEx GREEN ×1.00."""

NEW_HEADER = """// Cycle 2026-08-31 — FROZEN as '2026-08-31-v3.4.1-stageflip'. 12 names, ticker
...
# ---------------------------------------------------------------------------
OLD_TICKERS = "    tickers: string[]      // curated list; ALSO the fallback when no live portfolio is in the snapshot"
NEW_TICKERS = "    tickers: string[]      // DEAD as of 9/02 audit — SignalRecap renders name + editorial only"

OLD_WHOLE = "    wholeBucket?: boolean    // true: chips = ALL live holdings in `bucket` (auto add/drop). false/undefined: chips = `tickers` pruned to what's actually held."
NEW_WHOLE = "    wholeBucket?: boolean    // DEAD — grep for wholeBucket returns zero readers anywhere in src/. Kept as the spec for ticker chips, a feature specced here but never built. Do not trust these four fields to describe rendering."
...
def main():
    ...
    if "v3.4.1-stageflip" in src:
        fail("file already carries the v3.4.1 header -- patch appears to have run")

    print("Editing %s (%d lines)\n" % (TARGET, src.count("\n") + 1))

    for label, old, new in (
        ("header comment", OLD_HEADER, NEW_HEADER),
        ("tickers comment", OLD_TICKERS, NEW_TICKERS),
        ("wholeBucket comment", OLD_WHOLE, NEW_WHOLE),
    ):
        n = src.count(old)
        if n != 1:
            fail("%s found %d times, expected 1" % (label, n))
        src = src.replace(old, new)
        print("  %s updated" % label)

    print()
    for voice, body in (("Visser", VISSER), ("Camillo", CAMILLO), ("ZaStocks", ZASTOCKS)):
        src = swap_voice(src, voice, body)
