...
### Axis 4 — Application Dominance (persistent, franchise-driven) — BUILT & LIVE

Names: LLY, AMZN, HOOD. Scores app-layer names on **"how dominant does AI make this franchise"** instead of penalizing them on the bottleneck axis (where they always fail — they profit *from* the bottleneck, they aren't it). No trigger — persistent.

Rubric (proprietary-data-moat .30 / franchise-durability .25 / ai-efficiency-leverage .25 / tam-scale .20):

| name | data | durability | efficiency | tam | score | → weight |
|---|---|---|---|---|---|---|
| LLY | 75 | 72 | 60 | 63 | **68** | ~13.5% |
| AMZN | 60 | 62 | 62 | 55 | **60** | ~12.5% |
| HOOD | 55 | 55 | 65 | 72 | **61** | ~9% |

LLY 68 gives the deployed 13% flagship an **engine basis** (150-yr proprietary trial-data moat + own model), not a hand-override. AMZN's ai-efficiency dimension is haircut to 62 for Visser's hyperscaler-caution — the axis corrects what was a voice-floor *underweight* (deployed 7% → ~12.5%). App weight is score-driven with **no single-stock cap** (the old 12% cap was superseded 2026-06-01 by Rule B). Weight is insensitive to the exact score (±2 pts ≈ ±1%) — building the axis is what reprices these names.
...
- **First freeze — DONE (2026-07-07).** The four-axis book is live at `portfolio_version '2026-06-29-v3.0-themes.1'` (SOXX 15→4, AMZN 7→12.5, ASML 4→7, COPX 4.5→7.5; 12.5% one-way turnover). See `Thematic_Sleeve_Weights.md`.
