...
export const SYS_DETAILS: Record<string, SysDetail> = {
  ...
  themes: {
    ...
    blocks: [
      ...
      { t: 'note', text: 'theme_engine.py is a RECOMMENDER, not an authority. It reads three JSON inputs, prints, and writes theme_weights.json. It never writes the cron — you audit the output, then the freeze goes in by patch. Pure stdlib, no dependencies.' },
      { t: 'row', pill: 'open', tone: 'open', title: 'Silent-fallback risk', quote: 'load() returns a hardcoded default when an input file is missing rather than failing. If severity_scores.json is absent the run silently substitutes power .9 / chips .85 / memory .93 / interconnect .6 / copper .55 and still emits a plausible-looking recommendation. Same class as the null-DMA coercion: a default masquerading as a measurement.' },
    ],
    files: [
      F_THEME,
